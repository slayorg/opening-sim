...
function calculateScalar(scalingFactor: number, stat: number, base = 0) {
  return Math.floor(scalingFactor * stat / LEVEL_MOD + base) / 1000
}

// Damage formula
export function calculateDamage(potency: number, attackPower: number, weaponDamage: number, job: number) {
...
export function autoAttackPotency(weaponDelay: number, job: number) {
...
export function criticalRate(critical: number) {
  return calculateScalar(CRITICAL, critical - BASE_SUB, CRITICAL_CHANCE);
}

export function criticalMultiplier(critical: number) {
...
export function directHitRate(directHit: number) {
...
export function determinationMultiplier(determination: number) {
...
export function castTime(speed: number, baseCastTime = 250, modifiers = 1) {
